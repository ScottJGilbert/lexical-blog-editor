...
function MentionsTypeaheadMenuItem({ index, isSelected, onClick, onMouseEnter, option, }) {
    ...
    return (_jsxs("li", { tabIndex: -1, className: className, ref: option.setRefElement, role: "option", "aria-selected": isSelected, id: 'typeahead-item-' + index, onMouseEnter: onMouseEnter, onClick: onClick, children: [option.picture, _jsx("span", { className: "text", children: option.name })] }, option.key));
}
export default function NewMentionsPlugin() {
    ...
    const options = useMemo(() => results
        .map((result) => new MentionTypeaheadOption(result, _jsx("i", { className: "icon user" })))
    ...
    return (_jsx(LexicalTypeaheadMenuPlugin, { onQueryChange: setQueryString, onSelectOption: onSelectOption, triggerFn: checkForMentionMatch, options: options, menuRenderFn: (anchorElementRef, { selectedIndex, selectOptionAndCleanUp, setHighlightedIndex }) => anchorElementRef.current && results.length
            ? ReactDOM.createPortal(_jsx("div", { className: "typeahead-popover mentions-menu", children: _jsx("ul", { children: options.map((option, i) => (_jsx(MentionsTypeaheadMenuItem, { index: i, isSelected: selectedIndex === i, onClick: () => {
